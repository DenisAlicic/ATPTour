...
<title>Tennis Association: Deprecated!</title>
...
<div class="title">Deprecated! </div>  </div>
</div><!--header-->
<div class="contents">
<div class="textblock"><p class="">As of Feb 11th 2020, request is fully deprecated. No new changes are expected land. In fact, none have landed for some time.</p>
<p class="">For more information about why request is deprecated and possible alternatives refer to <a href="https://github.com/request/request/issues/3142">this issue</a>.</p>
<h1><a class="anchor" id="autotoc_md13265"></a>
Request - Simplified HTTP client</h1>
<p class=""><a href="https://nodei.co/npm/request/"><img src="https://nodei.co/npm/request.png?downloads=true&amp;downloadRank=true&amp;stars=true" alt="npm package" class="inline"/></a></p>
<p class=""><a href="https://travis-ci.org/request/request"><img src="https://img.shields.io/travis/request/request/master.svg?style=flat-square" alt="Build status" style="pointer-events: none;" class="inline"/></a> <a href="https://codecov.io/github/request/request?branch=master"><img src="https://img.shields.io/codecov/c/github/request/request.svg?style=flat-square" alt="Coverage" style="pointer-events: none;" class="inline"/></a> <a href="https://coveralls.io/r/request/request"><img src="https://img.shields.io/coveralls/request/request.svg?style=flat-square" alt="Coverage" style="pointer-events: none;" class="inline"/></a> <a href="https://david-dm.org/request/request"><img src="https://img.shields.io/david/request/request.svg?style=flat-square" alt="Dependency Status" style="pointer-events: none;" class="inline"/></a> <a href="https://snyk.io/test/npm/request"><img src="https://snyk.io/test/npm/request/badge.svg?style=flat-square" alt="Known Vulnerabilities" style="pointer-events: none;" class="inline"/></a> <a href="https://gitter.im/request/request?utm_source=badge"><img src="https://img.shields.io/badge/gitter-join_chat-blue.svg?style=flat-square" alt="Gitter" style="pointer-events: none;" class="inline"/></a></p>
<h2><a class="anchor" id="autotoc_md13266"></a>
Super simple to use</h2>
<p class="">Request is designed to be the simplest way possible to make http calls. It supports HTTPS and follows redirects by default.</p>
...
<div class="line">});</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md13267"></a>
...
<p class="">Request also offers <a href="#convenience-methods">convenience methods</a> like <code>request.defaults</code> and <code>request.post</code>, and there are lots of <a href="#examples">usage examples</a> and several <a href="#debugging">debugging techniques</a>.</p>
<hr  />
<h2><a class="anchor" id="autotoc_md13269"></a>
Streaming</h2>
<p class="">You can stream any response to a file stream.</p>
<div class="fragment"><div class="line">request(&#39;http://google.com/doodle.png&#39;).pipe(fs.createWriteStream(&#39;doodle.png&#39;))</div>
</div><!-- fragment --><p class="">You can also stream a file to a PUT or POST request. This method will also check the file extension against a mapping of file extensions to content-types (in this case <code>application/json</code>) and use the proper <code>content-type</code> in the PUT request (if the headers don’t already provide one).</p>
<div class="fragment"><div class="line">fs.createReadStream(&#39;file.json&#39;).pipe(request.put(&#39;http://mysite.com/obj.json&#39;))</div>
</div><!-- fragment --><p class="">Request can also <code>pipe</code> to itself. When doing so, <code>content-type</code> and <code>content-length</code> are preserved in the PUT headers.</p>
<div class="fragment"><div class="line">request.get(&#39;http://google.com/img.png&#39;).pipe(request.put(&#39;http://mysite.com/img.png&#39;))</div>
</div><!-- fragment --><p class="">Request emits a "response" event when a response is received. The <code>response</code> argument will be an instance of <a href="https://nodejs.org/api/http.html#http_class_http_incomingmessage">http.IncomingMessage</a>.</p>
...
</div><!-- fragment --><p class="">To easily handle errors when streaming requests, listen to the <code>error</code> event before piping:</p>
<div class="fragment"><div class="line">request</div>
...
</div><!-- fragment --><p class="">Now let’s get fancy.</p>
...
<div class="line">  }</div>
<div class="line">})</div>
</div><!-- fragment --><p class="">You can also <code>pipe()</code> from <code>http.ServerRequest</code> instances, as well as to <code>http.ServerResponse</code> instances. The HTTP method, headers, and entity-body data will be sent. Which means that, if you don't really care about security, you can do:</p>
<div class="fragment"><div class="line">http.createServer(function (req, resp) {</div>
<div class="line">  if (req.url === &#39;/doodle.png&#39;) {</div>
...
<div class="line">  }</div>
<div class="line">})</div>
</div><!-- fragment --><p class="">And since <code>pipe()</code> returns the destination stream in ≥ Node 0.5.x you can do one line proxying. :)</p>
<div class="fragment"><div class="line">req.pipe(request(&#39;http://mysite.com/doodle.png&#39;)).pipe(resp)</div>
</div><!-- fragment --><p class="">Also, none of this new functionality conflicts with requests previous features, it just expands them.</p>
...
<div class="line">  }</div>
<div class="line">})</div>
</div><!-- fragment --><p class="">You can still use intermediate proxies, the requests will still follow HTTP forwards, etc.</p>
<p class=""><a href="#table-of-contents">back to top</a></p>
<hr  />
<h2><a class="anchor" id="autotoc_md13271"></a>
Promises &amp; Async/Await</h2>
<p class=""><code>request</code> supports both streaming and callback interfaces natively. If you'd like <code>request</code> to return a Promise instead, you can use an alternative interface wrapper for <code>request</code>. These wrappers can be useful if you prefer to work with Promises, or if you'd like to use <code>async</code>/<code>await</code> in ES2017.</p>
<p class="">Several alternative interfaces are provided by the request team, including:</p><ul>
...
<p class="">Also, <a href="https://nodejs.org/api/util.html#util_util_promisify_original"><code>util.promisify</code></a>, which is available from Node.js v8.0 can be used to convert a regular function that takes a callback to return a promise instead.</p>
<p class=""><a href="#table-of-contents">back to top</a></p>
<hr  />
<h2><a class="anchor" id="autotoc_md13273"></a>
Forms</h2>
<p class=""><code>request</code> supports <code>application/x-www-form-urlencoded</code> and <code>multipart/form-data</code> form uploads. For <code>multipart/related</code> refer to the <code>multipart</code> API.</p>
<h4><a class="anchor" id="autotoc_md13274"></a>
application/x-www-form-urlencoded (URL-Encoded Forms)</h4>
<p class="">URL-encoded forms are simple.</p>
...
</div><!-- fragment --><h4><a class="anchor" id="autotoc_md13275"></a>
multipart/form-data (Multipart Form Uploads)</h4>
<p class="">For <code>multipart/form-data</code> we use the <a href="https://github.com/form-data/form-data">form-data</a> library by <a href="https://github.com/felixge">@felixge</a>. For the most cases, you can pass your upload form data via the <code>formData</code> option.</p>
...
<div class="line">});</div>
</div><!-- fragment --><p class="">For advanced cases, you can access the form-data object itself via <code>r.form()</code>. This can be modified until the request is fired on the next cycle of the event-loop. (Note that this calling <code>form()</code> will clear the currently set form data for that request.)</p>
...
<div class="line">  console.log(&#39;Upload successful!  Server responded with:&#39;, body);</div>
<div class="line">})</div>
</div><!-- fragment --><p class=""><a href="#table-of-contents">back to top</a></p>
<hr  />
...
<div class="line">  }</div>
<div class="line">});</div>
</div><!-- fragment --><p class="">If passed as an option, <code>auth</code> should be a hash containing values:</p>
<ul>
...
<p class="">The method form takes parameters <code>auth(username, password, sendImmediately, bearer)</code>.</p>
<p class=""><code>sendImmediately</code> defaults to <code>true</code>, which causes a basic or bearer authentication header to be sent. If <code>sendImmediately</code> is <code>false</code>, then <code>request</code> will retry with a proper authentication header after receiving a <code>401</code> response from the server (which must contain a <code>WWW-Authenticate</code> header indicating the required authentication method).</p>
<p class="">Note that you can also specify basic authentication using the URL itself, as detailed in <a href="http://www.ietf.org/rfc/rfc1738.txt">RFC 1738</a>. Simply pass the <code>user:password</code> before the host with an <code>@</code> sign:</p>
...
<div class="line">});</div>
</div><!-- fragment --><p class="">Digest authentication is supported, but it only works with <code>sendImmediately</code> set to <code>false</code>; otherwise <code>request</code> will send basic authentication on the initial request, which will probably cause the request to fail.</p>
<p class="">Bearer authentication is supported, and is activated when the <code>bearer</code> value is available. The value may be either a <code>String</code> or a <code>Function</code> returning a <code>String</code>. Using a function to supply the bearer token is particularly useful if used in conjunction with <code>defaults</code> to allow a single function to supply the last known token at the time of sending a request, or to compute one on the fly.</p>
<p class=""><a href="#table-of-contents">back to top</a></p>
<hr  />
<h2><a class="anchor" id="autotoc_md13280"></a>
Custom HTTP Headers</h2>
<p class="">HTTP Headers, such as <code>User-Agent</code>, can be set in the <code>options</code> object. In the example below, we call the github API to find out the number of stars and forks for the request repository. This requires a custom <code>User-Agent</code> header as well as https.</p>
<div class="fragment"><div class="line">const request = require(&#39;request&#39;);</div>
<div class="line"> </div>
...
<hr  />
<h2><a class="anchor" id="autotoc_md13282"></a>
OAuth Signing</h2>
<p class=""><a href="https://tools.ietf.org/html/rfc5849">OAuth version 1.0</a> is supported. The default signing algorithm is <a href="https://tools.ietf.org/html/rfc5849#section-3.4.2">HMAC-SHA1</a>:</p>
<div class="fragment"><div class="line">// OAuth1.0 - 3-legged server side flow (Twitter example)</div>
<div class="line">// step 1</div>
<div class="line">const qs = require(&#39;querystring&#39;)</div>
...
<div class="line">  const req_data = qs.parse(body)</div>
<div class="line">  const uri = &#39;https://api.twitter.com/oauth/authenticate&#39;</div>
...
<div class="line">  const auth_data = qs.parse(body)</div>
<div class="line">    , oauth =</div>
<div class="line">      { consumer_key: CONSUMER_KEY</div>
<div class="line">      , consumer_secret: CONSUMER_SECRET</div>
...
<div class="line">    const perm_data = qs.parse(body)</div>
<div class="line">      , oauth =</div>
<div class="line">        { consumer_key: CONSUMER_KEY</div>
<div class="line">        , consumer_secret: CONSUMER_SECRET</div>
...
<div class="line">  })</div>
<div class="line">})</div>
</div><!-- fragment --><p class="">For <a href="https://tools.ietf.org/html/rfc5849#section-3.4.3">RSA-SHA1 signing</a>, make the following changes to the OAuth options object:</p><ul>
...
<p class="">For <a href="http://oauth.net/core/1.0/#anchor22">PLAINTEXT signing</a>, make the following changes to the OAuth options object:</p><ul>
<li>Pass &lsquo;signature_method : 'PLAINTEXT&rsquo;`</li>
</ul>
<p class="">To send OAuth parameters via query params or in a post body as described in The <a href="http://oauth.net/core/1.0/#consumer_req_param">Consumer Request Parameters</a> section of the oauth1 spec:</p><ul>
...
<p class="">To use <a href="https://oauth.googlecode.com/svn/spec/ext/body_hash/1.0/oauth-bodyhash.html">Request Body Hash</a> you can either</p><ul>
...
Proxies</h2>
<p class="">If you specify a <code>proxy</code> option, then the request (and any subsequent redirects) will be sent via a connection to the proxy server.</p>
<p class="">If your endpoint is an <code>https</code> url, and you are using a proxy, then request will send a <code>CONNECT</code> request to the proxy server <em>first</em>, and then use the supplied connection to connect to the endpoint.</p>
<p class="">That is, first it will make a request like:</p>
...
</div><!-- fragment --><p class="">and then the proxy server make a TCP connection to <code>endpoint-server</code> on port <code>80</code>, and return a response that looks like:</p>
<div class="fragment"><div class="line">HTTP/1.1 200 OK</div>
</div><!-- fragment --><p class="">At this point, the connection is left open, and the client is communicating directly with the <code>endpoint-server.com</code> machine.</p>
<p class="">See <a href="https://en.wikipedia.org/wiki/HTTP_tunnel">the wikipedia page on HTTP Tunneling</a> for more information.</p>
<p class="">By default, when proxying <code>http</code> traffic, request will simply make a standard proxied <code>http</code> request. This is done by making the <code>url</code> section of the initial line of the request a fully qualified url to the endpoint.</p>
<p class="">For example, it will make a single request that looks like:</p>
...
</div><!-- fragment --><p class="">Because a pure "http over http" tunnel offers no additional security or other features, it is generally simpler to go with a straightforward HTTP proxy in this case. However, if you would like to force a tunneling proxy, you may set the <code>tunnel</code> option to <code>true</code>.</p>
<p class="">You can also make a standard proxied <code>http</code> request by explicitly setting <code>tunnel : false</code>, but <b>note that this will allow the proxy to see the traffic to/from the destination server</b>.</p>
<p class="">If you are using a tunneling proxy, you may set the <code>proxyHeaderWhiteList</code> to share certain headers with the proxy.</p>
<p class="">You can also set the <code>proxyHeaderExclusiveList</code> to share certain headers only with the proxy and not with destination host.</p>
<p class="">By default, this set is:</p>
...
</div><!-- fragment --><p class="">Note that, when using a tunneling proxy, the <code>proxy-authorization</code> header and any headers from custom <code>proxyHeaderExclusiveList</code> are <em>never</em> sent to the endpoint server, but only to the proxy server.</p>
<h3><a class="anchor" id="autotoc_md13285"></a>
Controlling proxy behaviour using environment variables</h3>
<p class="">The following environment variables are respected by <code>request</code>:</p>
<ul>
...
<p class=""><code>request</code> supports making requests to <a href="https://en.wikipedia.org/wiki/Unix_domain_socket">UNIX Domain Sockets</a>. To make one, use the following URL scheme:</p>
...
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md13290"></a>
Using &lt;tt&gt;options.agentOptions&lt;/tt&gt;</h3>
<p class="">In the example below, we call an API that requires client side SSL certificate (in PEM format) with passphrase protected private key (in PEM format) and disable the SSLv3 protocol:</p>
<div class="fragment"><div class="line">const fs = require(&#39;fs&#39;)</div>
<div class="line">    , path = require(&#39;path&#39;)</div>
<div class="line">    , certFile = path.resolve(__dirname, &#39;ssl/client.crt&#39;)</div>
<div class="line">    , keyFile = path.resolve(__dirname, &#39;ssl/client.key&#39;)</div>
...
<div class="line"> </div>
<div class="line">request.get(options);</div>
</div><!-- fragment --><p class="">It is able to force using SSLv3 only by specifying <code>secureProtocol</code>:</p>
...
<div class="line">    }</div>
<div class="line">});</div>
</div><!-- fragment --><p class="">The <code>ca</code> value can be an array of certificates, in the event you have a private or internal corporate public-key infrastructure hierarchy. For example, if you want to connect to <a href="https://api.some-server.com">https://api.some-server.com</a> which presents a key chain consisting of:</p><ol type="1">
...
<p class="">you can configure your request as follows:</p>
<div class="fragment"><div class="line">request.get({</div>
<div class="line">    url: &#39;https://api.some-server.com/&#39;,</div>
<div class="line">    agentOptions: {</div>
...
Support for HAR 1.2</h2>
<p class="">The <code>options.har</code> property will override the values: <code>url</code>, <code>method</code>, <code>qs</code>, <code>headers</code>, <code>form</code>, <code>formData</code>, <code>body</code>, <code>json</code>, as well as construct multipart data and read files from disk when <code>request.postData.params[].fileName</code> is present without a matching <code>value</code>.</p>
<p class="">A validation step will check if the HAR Request format matches the latest spec (v1.2) and will skip parsing if not matching.</p>
<div class="fragment"><div class="line">const request = require(&#39;request&#39;)</div>
...
request(options, callback)</h2>
<p class="">The first argument can be either a <code>url</code> or an <code>options</code> object. The only required option is <code>uri</code>; all others are optional.</p>
<ul>
...
<li><code>timeout</code> - integer containing number of milliseconds, controls two timeouts.<ul>
...
<li><code>har</code> - a <a href="http://www.softwareishard.com/blog/har-12-spec/#request">HAR 1.2 Request Object</a>, will be processed from HAR format into options overwriting matching values *(see the <a href="#support-for-har-12">HAR 1.2 section</a> for details)*</li>
<li><code>callback</code> - alternatively pass the request's callback in the options object</li>
</ul>
<p class="">The callback argument gets 3 arguments:</p>
<ol type="1">
...
Convenience methods</h2>
<p class="">There are also shorthand methods for different HTTP METHODs and some other conveniences.</p>
<h3><a class="anchor" id="autotoc_md13305"></a>
request.defaults(options)</h3>
<p class="">This method <b>returns a wrapper</b> around the normal request API that defaults to whatever options you pass to it.</p>
<p class=""><b>Note:</b> <code>request.defaults()</code> <b>does not</b> modify the global request API; instead, it <b>returns a wrapper</b> that has your default settings applied to it.</p>
<p class=""><b>Note:</b> You can call <code>.defaults()</code> on the wrapper that is returned from <code>request.defaults</code> to add/override defaults that were previously defaulted.</p>
<p class="">For example: </p><div class="fragment"><div class="line">//requests using baseRequest() will set the &#39;x-token&#39; header</div>
...
<div class="line">})</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md13306"></a>
request.METHOD()</h3>
<p class="">These HTTP method convenience functions act just like <code>request()</code> but with a default method already set for you:</p>
<ul>
...
<h3><a class="anchor" id="autotoc_md13307"></a>
request.cookie()</h3>
<p class="">Function that creates a new cookie.</p>
<div class="fragment"><div class="line">request.cookie(&#39;key1=value1&#39;)</div>
</div><!-- fragment --> <h3><a class="anchor" id="autotoc_md13308"></a>
request.jar()</h3>
<p class="">Function that creates a new cookie jar.</p>
<div class="fragment"><div class="line">request.jar()</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md13309"></a>
response.caseless.get('header-name')</h3>
<p class="">Function that returns the specified response header field using a <a href="https://tools.ietf.org/html/rfc7230#section-3.2">case-insensitive match</a></p>
...
<hr  />
<h2><a class="anchor" id="autotoc_md13311"></a>
Debugging</h2>
<p class="">There are at least three ways to debug the operation of <code>request</code>:</p>
<ol type="1">
...
<p class=""><a href="#table-of-contents">back to top</a></p>
<hr  />
<h2><a class="anchor" id="autotoc_md13313"></a>
Timeouts</h2>
<p class="">Most requests to external servers should have a timeout attached, in case the server is not responding in a timely manner. Without a timeout, your code may have a socket open/consume resources for minutes or more.</p>
<p class="">There are two main types of timeouts: <b>connection timeouts</b> and <b>read timeouts</b>. A connect timeout occurs if the timeout is hit while your client is attempting to establish a connection to a remote machine (corresponding to the <a href="http://linux.die.net/man/2/connect">connect() call</a> on the socket). A read timeout occurs any time the server is too slow to send back a part of the response.</p>
...
<div class="line">  })</div>
<div class="line">})</div>
</div><!-- fragment --><p class="">Cookies are disabled by default (else, they would be used in subsequent requests). To enable cookies, set <code>jar</code> to <code>true</code> (either in <code>defaults</code> or <code>options</code>).</p>
...
<div class="line">})</div>
</div><!-- fragment --><p class="">To use a custom cookie jar (instead of <code>request</code>’s global cookie jar), set <code>jar</code> to an instance of <code>request.jar()</code> (either in <code>defaults</code> or <code>options</code>)</p>
...
<div class="line">  request(&#39;http://images.google.com&#39;)</div>
<div class="line">})</div>
</div><!-- fragment --><p class="">To use a custom cookie store (such as a <a href="https://github.com/mitsuru/tough-cookie-filestore"><code>FileCookieStore</code></a> which supports saving to and restoring from JSON files), pass it as a parameter to <code>request.jar()</code>:</p>
...
<div class="line">  request(&#39;http://images.google.com&#39;)</div>
<div class="line">})</div>
</div><!-- fragment --><p class="">The cookie store must be a <a href="https://github.com/SalesforceEng/tough-cookie"><code>tough-cookie</code></a> store and it must support synchronous operations; see the <a href="https://github.com/SalesforceEng/tough-cookie#api"><code>CookieStore</code> API docs</a> for details.</p>
<p class="">To inspect your cookie jar after a request:</p>
<div class="fragment"><div class="line">const j = request.jar()</div>
