...
<div class="contents">
<div class="textblock"><p class=""><a href="http://badge.fury.io/js/sockjs"><img src="https://badge.fury.io/js/sockjs.svg" alt="NPM version" style="pointer-events: none;" class="inline"/></a></p>
<p class="">SockJS family:</p>
<ul>
...
What is SockJS?</h1>
<p class="">SockJS is a JavaScript library (for browsers) that provides a WebSocket-like object. SockJS gives you a coherent, cross-browser, Javascript API which creates a low latency, full duplex, cross-domain communication channel between the browser and the web server, with WebSockets or without. This necessitates the use of a server, which this is one version of, for Node.js.</p>
<h1><a class="anchor" id="autotoc_md21464"></a>
SockJS-node server</h1>
<p class="">SockJS-node is a Node.js server side counterpart of <a href="https://github.com/sockjs/sockjs-client">SockJS-client browser library</a> written in CoffeeScript.</p>
<p class="">To install <code>sockjs-node</code> run: </p><pre class="fragment">npm install sockjs
...
</div><!-- fragment --><p class="">(Take look at <a href="https://github.com/sockjs/sockjs-node/tree/master/examples/echo">examples</a> directory for a complete version.)</p>
<p class="">Subscribe to <a href="https://groups.google.com/forum/#!forum/sockjs">SockJS mailing list</a> for discussions and support.</p>
<h2><a class="anchor" id="autotoc_md21465"></a>
SockJS-node API</h2>
<p class="">The API design is based on common Node APIs like the <a href="http://nodejs.org/docs/v0.5.8/api/streams.html">Streams API</a> or the <a href="http://nodejs.org/docs/v0.5.8/api/http.html#http.Server">Http.Server API</a>.</p>
<h3><a class="anchor" id="autotoc_md21466"></a>
Server class</h3>
<p class="">SockJS module is generating a <code>Server</code> class, similar to <a href="http://nodejs.org/docs/v0.5.8/api/http.html#http.createServer">Node.js http.createServer</a> module.</p>
<div class="fragment"><div class="line">var sockjs_server = sockjs.createServer(options);</div>
</div><!-- fragment --><p class="">Where <code>options</code> is a hash which can contain:</p>
<dl>
...
<h3><a class="anchor" id="autotoc_md21467"></a>
Server instance</h3>
<p class="">Once you have create <code>Server</code> instance you can hook it to the <a href="http://nodejs.org/docs/v0.5.8/api/http.html#http.createServer">http.Server instance</a>.</p>
...
</div><!-- fragment --><p class="">Where <code>options</code> can overshadow options given when creating <code>Server</code> instance.</p>
<p class=""><code>Server</code> instance is an <a href="http://nodejs.org/docs/v0.4.10/api/events.html#events.EventEmitter">EventEmitter</a>, and emits following event:</p>
<dl>
<dt>Event: connection (connection) </dt>
<dd>A new connection has been successfully opened. </dd>
</dl>
<p class="">All http requests that don't go under the path selected by <code>prefix</code> will remain unanswered and will be passed to previously registered handlers. You must install your custom http handlers before calling <code>installHandlers</code>.</p>
<h3><a class="anchor" id="autotoc_md21468"></a>
Connection instance</h3>
<p class="">A <code>Connection</code> instance supports <a href="http://nodejs.org/docs/v0.5.8/api/streams.html">Node Stream API</a> and has following methods and properties:</p>
<dl>
...
<p class="">A <code>Connection</code> instance emits the following events:</p>
<dl>
...
<div class="line">    });</div>
<div class="line">});</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md21469"></a>
Footnote</h3>
<p class="">A fully working echo server does need a bit more boilerplate (to handle requests unanswered by SockJS), see the <a href="https://github.com/sockjs/sockjs-node/tree/master/examples/echo"><code>echo</code> example</a> for a complete code.</p>
<h3><a class="anchor" id="autotoc_md21470"></a>
Examples</h3>
<p class="">If you want to see samples of running code, take a look at:</p>
<ul>
...
<p class="">Although the main point of SockJS it to enable browser-to-server connectivity, it is possible to connect to SockJS from an external application. Any SockJS server complying with 0.3 protocol does support a raw WebSocket url. The raw WebSocket url for the test server looks like:</p>
<ul>
<li>ws://localhost:8081/echo/websocket</li>
</ul>
<p class="">You can connect any WebSocket RFC 6455 compliant WebSocket client to this url. This can be a command line client, external application, third party code or even a browser (though I don't know why you would want to do so).</p>
<p class="">Note: This endpoint will <em>not send any heartbeat packets</em>.</p>
<h2><a class="anchor" id="autotoc_md21472"></a>
Deployment and load balancing</h2>
<p class="">There are two issues that need to be considered when planning a non-trivial SockJS-node deployment: WebSocket-compatible load balancer and sticky sessions (aka session affinity).</p>
<h3><a class="anchor" id="autotoc_md21473"></a>
WebSocket compatible load balancer</h3>
<p class="">Often WebSockets don't play nicely with proxies and load balancers. Deploying a SockJS server behind Nginx or Apache could be painful.</p>
<p class="">Fortunately recent versions of an excellent load balancer <a href="http://haproxy.1wt.eu/">HAProxy</a> are able to proxy WebSocket connections. We propose to put HAProxy as a front line load balancer and use it to split SockJS traffic from normal HTTP data. Take a look at the sample <a href="https://github.com/sockjs/sockjs-node/blob/master/examples/haproxy.cfg">SockJS HAProxy configuration</a>.</p>
<p class="">The config also shows how to use HAproxy balancing to split traffic between multiple Node.js servers. You can also do balancing using dns names.</p>
<h3><a class="anchor" id="autotoc_md21474"></a>
Sticky sessions</h3>
<p class="">If you plan deploying more than one SockJS server, you must make sure that all HTTP requests for a single session will hit the same server. SockJS has two mechanisms that can be useful to achieve that:</p>
<ul>
...
<p class="">If you want to work on SockJS-node source code, you need to clone the git repo and follow these steps. First you need to install dependencies: </p><pre class="fragment">cd sockjs-node
npm install
npm install --dev
...
</pre> <h3><a class="anchor" id="autotoc_md21476"></a>
SockJS-protocol Python tests</h3>
<p class="">To run it run something like: </p><pre class="fragment">cd sockjs-protocol
make test_deps
...
<h3><a class="anchor" id="autotoc_md21477"></a>
SockJS-client QUnit tests</h3>
<p class="">You need to start a second web server (by default listening on 8080) that is serving various static html and javascript files: </p><pre class="fragment">cd sockjs-client
make test
...
<p class="">For details see <a href="https://github.com/sockjs/sockjs-client#readme">SockJS-client README</a>.</p>
<p class="">Additionally, if you're doing more serious development consider using <code>make serve</code>, which will automatically the server when you modify the source code.</p>
<h2><a class="anchor" id="autotoc_md21478"></a>
Various issues and design considerations</h2>
<h3><a class="anchor" id="autotoc_md21479"></a>
Authorisation</h3>
<p class="">SockJS-node does not expose cookies to the application. This is done deliberately as using cookie-based authorisation with SockJS simply doesn't make sense and will lead to security issues.</p>
<p class="">Cookies are a contract between a browser and an http server, and are identified by a domain name. If a browser has a cookie set for particular domain, it will pass it as a part of all http requests to the host. But to get various transports working, SockJS uses a middleman</p><ul>
...
<p class="">Basically - cookies are not suited for SockJS model. If you want to authorise a session - provide a unique token on a page, send it as a first thing over SockJS connection and validate it on the server side. In essence, this is how cookies work.</p>
<h3><a class="anchor" id="autotoc_md21480"></a>
Deploying SockJS on Heroku</h3>
<p class="">Long polling is known to cause problems on Heroku, but <a href="https://github.com/sockjs/sockjs-node/issues/57#issuecomment-5242187">workaround for SockJS is available</a>. </p>
</div></div><!-- contents -->
</div><!-- PageDoc -->
<!-- start footer part -->
