...
<div class="contents">
<div class="textblock"><p class=""><a href="https://github.com/zkat/protoduck"><code>protoduck</code></a> is a JavaScript library is a library for making groups of methods, called "protocols".</p>
<p class="">If you're familiar with the concept of <a href="https://en.wikipedia.org/wiki/Duck_typing">"duck typing"</a>, then it might make sense to think of protocols as things that explicitly define what methods you need in order to "clearly be a duck".</p>
<h1><a class="anchor" id="autotoc_md21748"></a>
Install</h1>
<p class=""><code>$ npm install -S protoduck</code></p>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md21751"></a>
Features</h2>
<ul>
...
<h2><a class="anchor" id="autotoc_md21752"></a>
Guide</h2>
<h3><a class="anchor" id="autotoc_md21753"></a>
Introduction</h3>
<p class="">Like most Object-oriented languages, JavaScript comes with its own way of defining methods: You simply add regular <code>function</code>s as properties to regular objects, and when you do <code>obj.method()</code>, it calls the right code! ES6/ES2015 further extended this by adding a <code>class</code> syntax that allowed this same system to work with more familiar syntax sugar: <code>class Foo { method() { ... } }</code>.</p>
<p class="">The point of "protocols" is to have a more explicit definitions of what methods "go together". That is, a protocol is a description of a type of object your code interacts with. If someone passes an object into your library, and it fits your defined protocol, the assumption is that the object will work just as well.</p>
<p class="">Duck typing is a common term for this sort of thing: If it walks like a duck, and it talks like a duck, then it may as well be a duck, as far as any of our code is concerned.</p>
<p class="">Many other languages have similar or identical concepts under different names: Java's interfaces, Haskell's typeclasses, Rust's traits. Elixir and Clojure both call them "protocols" as well.</p>
<p class="">One big advantage to using these protocols is that they let users define their own versions of some abstraction, without requiring the type to inherit from another &ndash; protocols are independent of inheritance, even though they're able to work together with it. If you've ever found yourself in some sort of inheritance mess, this is exactly the sort of thing you use to escape it.</p>
<h3><a class="anchor" id="autotoc_md21754"></a>
Defining Protocols</h3>
<p class="">The first step to using <code>protoduck</code> is to define a protocol. Protocol definitions look like this:</p>
...
<div class="line">})</div>
</div><!-- fragment --><p class="">Protocols by themselves don't really <em>do</em> anything, they simply define what methods are included in the protocol, and thus what will need to be implemented.</p>
<h3><a class="anchor" id="autotoc_md21755"></a>
Protocol Impls</h3>
<p class="">The simplest type of definitions for protocols are as regular methods. In this style, protocols end up working exactly like normal JavaScript methods: they're added as properties of the target type/object, and we call them using the <code>foo.method()</code> syntax. <code>this</code> is accessible inside the methods, as usual.</p>
<p class="">Implementation syntax is very similar to protocol definitions, using <code>.impl</code>:</p>
...
<div class="line">})</div>
</div><!-- fragment --><p class="">So now, our <code>Dog</code> class has two extra methods: <code>walk</code>, and <code>talk</code>, and we can just call them:</p>
...
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md21756"></a>
Multiple Dispatch</h3>
<p class="">You may have noticed before that we have these <code>[]</code> in various places that don't seem to have any obvious purpose.</p>
<p class="">These arrays allow protocols to be implemented not just for a single value of <code>this</code>, but across <em>all arguments</em>. That is, you can have methods in these protocols that use both <code>this</code>, and the first argument (or any other arguments) in order to determine what code to actually execute.</p>
<p class="">This type of method is called a multimethod, and is one of the differences between protoduck and the default <code>class</code> syntax.</p>
<p class="">To use it: in the protocol <em>definitions</em>, you put matching strings in different spots where those empty arrays were, and when you <em>implement</em> the protocol, you give the definition the actual types/objects you want to implement it on, and it takes care of mapping types to the strings you defined, and making sure the right code is run:</p>
...
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md21757"></a>
Constraints</h3>
<p class="">Sometimes, you want to have all the functionality of a certain protocol, but you want to add a few requirements or other bits an pieces. Usually, you would have to define the entire functionality of the "parent" protocol in your own protocol in order to pull this off. This isn't very DRY and thus prone to errors, missing or out-of-sync functionality, or other issues. You could also just tell users "hey, if you implement this, make sure to implement that", but there's no guarantee they'll know about it, or know which arguments map to what.</p>
<p class="">This is where constraints come in: You can define a protocol that expects anything that implements it to <em>also</em> implement one or more "parent" protocols.</p>
...
<div class="line">Log.impl(MyThing)</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md21758"></a>
API</h2>
<h3><a class="anchor" id="autotoc_md21759"></a>
&lt;a name="define"&gt;&lt;/a&gt; &lt;tt&gt;define(&lt;types&gt;?, &lt;spec&gt;, &lt;opts&gt;)&lt;/tt&gt;</h3>
<p class="">Defines a new protocol on across arguments of types defined by <code>&lt;types&gt;</code>, which will expect implementations for the functions specified in <code>&lt;spec&gt;</code>.</p>
<p class="">If <code>&lt;types&gt;</code> is missing, it will be treated the same as if it were an empty array.</p>
<p class="">The types in <code>&lt;spec&gt;</code> entries must map, by string name, to the type names specified in <code>&lt;types&gt;</code>, or be an empty array if <code>&lt;types&gt;</code> is omitted. The types in <code>&lt;spec&gt;</code> will then be used to map between method implementations for the individual functions, and the provided types in the impl.</p>
<p class="">Protocols can include an <code>opts</code> object as the last argument, with the following available options:</p>
<ul>
...
<h4><a class="anchor" id="autotoc_md21760"></a>
Example</h4>
...
<p class="">Adds a new implementation to the given protocol across <code>&lt;types&gt;</code>.</p>
<p class=""><code>&lt;implementations&gt;</code> must be an object with functions matching the protocol's API. If given, the types in <code>&lt;types&gt;</code> will be mapped to their corresponding method arguments according to the original protocol definition.</p>
<p class="">If a protocol is derivable &ndash; that is, all its functions have default impls, then the <code>&lt;implementations&gt;</code> object can be omitted entirely, and the protocol will be automatically derived for the given <code>&lt;types&gt;</code></p>
<h4><a class="anchor" id="autotoc_md21762"></a>
Example</h4>
