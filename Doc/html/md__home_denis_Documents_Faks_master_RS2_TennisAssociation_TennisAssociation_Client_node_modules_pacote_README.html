...
<h2><a class="anchor" id="autotoc_md22573"></a>
Example</h2>
...
<div class="line">})</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22574"></a>
Features</h2>
<ul>
...
<p class="">Fetches the <em>manifest</em> for a package. Manifest objects are similar and based on the <code>package.json</code> for that package, but with pre-processed and limited fields. The object has the following shape:</p>
...
<div class="line">}</div>
</div><!-- fragment --><p class="">Note that depending on the spec type, some additional fields might be present. For example, packages from <code>registry.npmjs.org</code> have additional metadata appended by the registry.</p>
<h4><a class="anchor" id="autotoc_md22578"></a>
Example</h4>
...
<div class="line">})</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md22579"></a>
&lt;a name="packument"&gt;&lt;/a&gt; &lt;tt&gt;&gt; pacote.packument(spec, [opts])&lt;/tt&gt;</h3>
<p class="">Fetches the <em>packument</em> for a package. Packument objects are general metadata about a project corresponding to registry metadata, and include version and <code>dist-tag</code> information about a package's available versions, rather than a specific version. It may include additional metadata not usually available through the individual package metadata objects.</p>
<p class="">It generally looks something like this:</p>
<div class="fragment"><div class="line">{</div>
<div class="line">  &quot;name&quot;: PkgName,</div>
...
<h4><a class="anchor" id="autotoc_md22580"></a>
Example</h4>
...
<div class="line">})</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md22581"></a>
&lt;a name="extract"&gt;&lt;/a&gt; &lt;tt&gt;&gt; pacote.extract(spec, destination, [opts])&lt;/tt&gt;</h3>
<p class="">Extracts package data identified by <code>&lt;spec&gt;</code> into a directory named <code>&lt;destination&gt;</code>, which will be created if it does not already exist.</p>
<p class="">If <code>opts.digest</code> is provided and the data it identifies is present in the cache, <code>extract</code> will bypass most of its operations and go straight to extracting the tarball.</p>
<h4><a class="anchor" id="autotoc_md22582"></a>
Example</h4>
...
<div class="line">})</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md22583"></a>
&lt;a name="tarball"&gt;&lt;/a&gt; &lt;tt&gt;&gt; pacote.tarball(spec, [opts])&lt;/tt&gt;</h3>
<p class="">Fetches package data identified by <code>&lt;spec&gt;</code> and returns the data as a buffer.</p>
<p class="">This API has two variants:</p>
<ul>
...
<h4><a class="anchor" id="autotoc_md22584"></a>
Example</h4>
...
<div class="line">})</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md22585"></a>
&lt;a name="tarball-stream"&gt;&lt;/a&gt; &lt;tt&gt;&gt; pacote.tarball.stream(spec, [opts])&lt;/tt&gt;</h3>
<p class="">Same as <code>pacote.tarball</code>, except it returns a stream instead of a Promise.</p>
<h4><a class="anchor" id="autotoc_md22586"></a>
Example</h4>
...
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md22587"></a>
&lt;a name="tarball-to-file"&gt;&lt;/a&gt; &lt;tt&gt;&gt; pacote.tarball.toFile(spec, dest, [opts])&lt;/tt&gt;</h3>
<p class="">Like <code>pacote.tarball</code>, but instead of returning data directly, data will be written directly to <code>dest</code>, and create any required directories along the way.</p>
<h4><a class="anchor" id="autotoc_md22588"></a>
Example</h4>
...
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md22589"></a>
&lt;a name="prefetch"&gt;&lt;/a&gt; &lt;tt&gt;&gt; pacote.prefetch(spec, [opts])&lt;/tt&gt;</h3>
<h4><a class="anchor" id="autotoc_md22590"></a>
THIS API IS DEPRECATED. USE &lt;tt&gt;pacote.tarball()&lt;/tt&gt; INSTEAD</h4>
<p class="">Fetches package data identified by <code>&lt;spec&gt;</code>, usually for the purpose of warming up the local package cache (with <code>opts.cache</code>). It does not return anything.</p>
<h4><a class="anchor" id="autotoc_md22591"></a>
Example</h4>
...
<div class="line">})</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md22592"></a>
&lt;a name="clearMemoized"&gt;&lt;/a&gt; &lt;tt&gt;&gt; pacote.clearMemoized()&lt;/tt&gt;</h3>
<p class="">This utility function can be used to force pacote to release its references to any memoized data in its various internal caches. It might help free some memory.</p>
<div class="fragment"><div class="line">pacote.manifest(...).then(() =&gt; pacote.clearMemoized)</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md22593"></a>
&lt;a name="options"&gt;&lt;/a&gt; &lt;tt&gt;&gt; options&lt;/tt&gt;</h3>
<p class=""><code>pacote</code> accepts <a href="https://npm.im/npm-registry-fetch#fetch-options">the options for <code>npm-registry-fetch</code></a> as-is, with a couple of additional <code>pacote-specific</code> ones:</p>
<h4><a class="anchor" id="autotoc_md22594"></a>
...
<p class="">Expects a function that takes a single argument, <code>dir</code>, and returns a <code>ReadableStream</code> that outputs packaged tarball data. Used when creating tarballs for package specs that are not already packaged, such as git and directory dependencies. The default <code>opts.dirPacker</code> does not execute <code>prepare</code> scripts, even though npm itself does.</p>
<h4><a class="anchor" id="autotoc_md22595"></a>
...
<p class="">If passed in, will be used while resolving to filter the versions for <b>registry dependencies</b> such that versions published <b>after</b> <code>opts.enjoy-by</code> are not considered &ndash; as if they'd never been published.</p>
<h4><a class="anchor" id="autotoc_md22596"></a>
...
<p class="">If false, deprecated versions will be skipped when selecting from registry range specifiers. If true, deprecations do not affect version selection.</p>
<h4><a class="anchor" id="autotoc_md22597"></a>
&lt;a name="opts-full-metadata"&gt;&lt;/a&gt; &lt;tt&gt;opts.full-metadata&lt;/tt&gt;</h4>
<ul>
<li>Type: Boolean</li>
<li>Default: false</li>
</ul>
<p class="">If <code>true</code>, the full packument will be fetched when doing metadata requests. By defaul, <code>pacote</code> only fetches the summarized packuments, also called "corgis".</p>
<h4><a class="anchor" id="autotoc_md22598"></a>
...
<p class="">Package version resolution tag. When processing registry spec ranges, this option is used to determine what dist-tag to treat as "latest". For more details about how <code>pacote</code> selects versions and how <code>tag</code> is involved, see <a href="https://npm.im/npm-pick-manifest">the documentation for <code>npm-pick-manifest</code></a>.</p>
<h4><a class="anchor" id="autotoc_md22599"></a>
&lt;a name="opts-resolved"&gt;&lt;/a&gt; &lt;tt&gt;opts.resolved&lt;/tt&gt;</h4>
<ul>
<li>Type: String</li>
<li>Default: null</li>
</ul>
<p class="">When fetching tarballs, this option can be passed in to skip registry metadata lookups when downloading tarballs. If the string is a <code>file:</code> URL, pacote will try to read the referenced local file before attempting to do any further lookups. This option does not bypass integrity checks when <code>opts.integrity</code> is passed in.</p>
<h4><a class="anchor" id="autotoc_md22600"></a>
&lt;a name="opts-where"&gt;&lt;/a&gt; &lt;tt&gt;opts.where&lt;/tt&gt;</h4>
<ul>
<li>Type: String</li>
<li>Default: null</li>
</ul>
<p class="">Passed as an argument to <a href="https://npm.im/npm-package-arg"><code>npm-package-arg</code></a> when resolving <code>spec</code> arguments. Used to determine what path to resolve local path specs relatively from. </p>
</div></div><!-- contents -->
</div><!-- PageDoc -->
<!-- start footer part -->
