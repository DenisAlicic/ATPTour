...
<title>Tennis Association: pacote</title>
...
<div class="title">pacote </div>  </div>
</div><!--header-->
<div class="contents">
<div class="textblock"><p class="">Fetches package manifests and tarballs from the npm registry.</p>
<h1><a class="anchor" id="autotoc_md15369"></a>
USAGE</h1>
...
<div class="line">})</div>
</div><!-- fragment --><p class=""><code>pacote</code> works with any kind of package specifier that npm can install. If you can pass it to the npm CLI, you can pass it to pacote. (In fact, that's exactly what the npm CLI does.)</p>
<p class="">Anything that you can do with one kind of package, you can do with another.</p>
<p class="">Data that isn't relevant (like a packument for a tarball) will be simulated.</p>
<p class=""><code>prepare</code> scripts will be run when generating tarballs from <code>git</code> and <code>directory</code> locations, to simulate what <em>would</em> be published to the registry, so that you get a working package instead of just raw source code that might need to be transpiled.</p>
<h1><a class="anchor" id="autotoc_md15370"></a>
CLI</h1>
<p class="">This module exports a command line interface that can do most of what is described below. Run <code>pacote -h</code> to learn more.</p>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md15371"></a>
API</h1>
<p class="">The <code>spec</code> refers to any kind of package specifier that npm can install. If you can pass it to the npm CLI, you can pass it to pacote. (In fact, that's exactly what the npm CLI does.)</p>
<p class="">See below for valid <code>opts</code> values.</p>
<ul>
...
Options</h2>
<p class="">Options are passed to <a href="http://npm.im/npm-registry-fetch"><code>npm-registry-fetch</code></a> and <a href="http://npm.im/cacache"><code>cacache</code></a>, so in addition to these, anything for those modules can be given to pacote as well.</p>
<p class="">Options object is cloned, and mutated along the way to add integrity, resolved, and other properties, as they are determined.</p>
<ul>
...
Advanced API</h2>
<p class="">Each different type of fetcher is exposed for more advanced usage such as using helper methods from this classes:</p>
<ul>
...
<p class="">Files are extracted with a mode matching the following formula:</p>
<div class="fragment"><div class="line">( (tarball entry mode value) | (minimum mode option) ) ~ (umask)</div>
</div><!-- fragment --><p class="">This is in order to prevent unreadable files or unlistable directories from cluttering a project's <code>node_modules</code> folder, even if the package tarball specifies that the file should be inaccessible.</p>
<p class="">It also prevents files from being group- or world-writable without explicit opt-in by the user, because all file and directory modes are masked against the <code>umask</code> value.</p>
<p class="">So, a file which is <code>0o771</code> in the tarball, using the default <code>fmode</code> of <code>0o666</code> and <code>umask</code> of <code>0o22</code>, will result in a file mode of <code>0o755</code>:</p>
<div class="fragment"><div class="line">(0o771 | 0o666) =&gt; 0o777</div>
<div class="line">(0o777 ~ 0o22) =&gt; 0o755</div>
</div><!-- fragment --><p class="">In almost every case, the defaults are appropriate. To respect exactly what is in the package tarball (even if this makes an unusable system), set both <code>dmode</code> and <code>fmode</code> options to <code>0</code>. Otherwise, the <code>umask</code> config should be used in most cases where file mode modifications are required, and this functions more or less the same as the <code>umask</code> value in most Unix systems.</p>
<h1><a class="anchor" id="autotoc_md15375"></a>
Extracted File Ownership</h1>
<p class="">When running as <code>root</code> on Unix systems, all extracted files and folders will have their owning <code>uid</code> and <code>gid</code> values set to match the ownership of the containing folder.</p>
<p class="">This prevents <code>root</code>-owned files showing up in a project's <code>node_modules</code> folder when a user runs <code>sudo npm install</code>.</p>
<h1><a class="anchor" id="autotoc_md15376"></a>
Manifests</h1>
<p class="">A <code>manifest</code> is similar to a <code>package.json</code> file. However, it has a few pieces of extra metadata, and sometimes lacks metadata that is inessential to package installation.</p>
<p class="">In addition to the common <code>package.json</code> fields, manifests include:</p>
<ul>
...
Packuments</h1>
<p class="">A packument is the top-level package document that lists the set of manifests for available versions for a package.</p>
<p class="">When a packument is fetched with <code>accept: application/vnd.npm.install-v1+json</code> in the HTTP headers, only the most minimum necessary metadata is returned. Additional metadata is returned when fetched with only <code>accept: application/json</code>.</p>
<p class="">For Pacote's purposes, the following fields are relevant:</p>
<ul>
