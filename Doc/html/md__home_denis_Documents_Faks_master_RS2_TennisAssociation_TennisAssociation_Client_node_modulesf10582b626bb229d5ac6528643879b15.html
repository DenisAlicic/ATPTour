...
<div class="contents">
<div class="textblock"><h1><a class="anchor" id="autotoc_md10717"></a>
Example for SOCKS 'associate' command</h1>
<p class="">The associate command tells the SOCKS proxy server to establish a UDP relay. The server binds to a new UDP port and communicates the newly opened port back to the origin client. From here, any SOCKS UDP frame packets sent to this special UDP port on the Proxy server will be forwarded to the desired destination, and any responses will be forwarded back to the origin client (you).</p>
<p class="">This can be used for things such as DNS queries, and other UDP communicates.</p>
<p class=""><b>Connection Steps</b></p>
<ol type="1">
...
<p class="">At this point the proxy is accepting UDP frames on the specified port.</p>
<ol type="1">
...
<h1><a class="anchor" id="autotoc_md10718"></a>
Usage</h1>
<p class="">The 'associate' command can only be used by creating a new SocksClient instance and listening for the 'established' event.</p>
<p class=""><b>Note:</b> UDP packets relayed through the proxy servers are encompassed in a special Socks UDP frame format. SocksClient.createUDPFrame() and SocksClient.parseUDPFrame() create and parse these special UDP packets.</p>
