...
<title>Tennis Association: Description</title>
...
<div class="contents">
<div class="textblock"><p class="">Jobs is the Angular DevKit subsystem for scheduling and running generic functions with clearly typed inputs and outputs. A <code>Job</code> instance is a function associated with metadata. You can schedule a job, synchronize it with other jobs, and use it to schedule other jobs.</p>
<p class="">The whole API is serializable, allowing you to use a Node Stream or message channel to communicate between the job and the job scheduler.</p>
<p class="">Jobs are lazy, cold, and guaranteed to execute exactly once when scheduled. Subscribing to a job returns messages from the point where the job is at.</p>
<h1><a class="anchor" id="autotoc_md20306"></a>
Argument, Input, Output and Channels</h1>
<p class="">A job receives a single argument when scheduled and can also listen to an input channel. It can emit multiple outputs, and can also provide multiple output channels that emit asynchronous JSON messages, which can be typed.</p>
<p class="">The I/O model is like that of an executable, where the argument corresponds to arguments on the command line, the input channel to STDIN, the output channel to STDOUT, and the channels would be additional output streams.</p>
<h1><a class="anchor" id="autotoc_md20307"></a>
LifeCycle</h1>
<p class="">A <code>Job</code> goes through multiple LifeCycle messages before its completion;</p><ol type="1">
...
<p class="">Each state (except <code>Queued</code>) corresponds to a <code>JobOutboundMessage</code> on the <code>outboundBus</code> observable that triggers the state change. The <code>Scheduler</code> emits the <code>Ready</code> and <code>Errored</code> messages; the job implementation should not emit them, and if it does they are filtered out. You can listen for these messages or use the corresponding state member.</p>
<p class="">The job implementation should emit the <code>Start</code> and <code>End</code> messages when it is starting the job logic itself. Only the first <code>Start</code> and <code>End</code> messages will be forwarded. Any more will be filtered out.</p>
<p class="">The <code>Queued</code> state is set as the job is scheduled, so there is no need to listen for the message.</p>
<h1><a class="anchor" id="autotoc_md20308"></a>
&lt;tt&gt;Job&lt;OutputType&gt;&lt;/tt&gt; Object</h1>
<p class="">The <code>Job</code> object that is returned when you schedule a job provides access to the job's status and utilities for tracking and modifying the job.</p>
<ol type="1">
...
<h1><a class="anchor" id="autotoc_md20309"></a>
&lt;tt&gt;JobHandlerContext&lt;I, O&gt;&lt;/tt&gt; Object</h1>
<p class="">The <code>JobHandlerContext&lt;&gt;</code> is passed to the job handler code in addition to its argument. The context contains the following members:</p>
<ol type="1">
...
<h1><a class="anchor" id="autotoc_md20310"></a>
Examples</h1>
<p class="">An example of a job that adds all input together and return the output value. We use a simple synchronous job registry and a simple job scheduler.</p>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md20311"></a>
Creating Jobs</h1>
<p class="">A job is at its core a function with a description object attached to it. The description object stores the JSON schemas used to validate the types of the argument passed in, the input and output values. By default, a job accepts and can output any JSON object.</p>
...
</div><!-- fragment --><p class="">This is a lot of boilerplate, so we made some helpers to improve readability and manage argument, input and output automatically:</p>
...
</div><!-- fragment --><p class="">You can also return a Promise or an Observable, as jobs are asynchronous. This helper will set start and end messages appropriately. It will also manage channels automatically (see below).</p>
<p class="">A more complex job can be declared like this:</p>
<div class="fragment"><div class="line">import { Observable } from &#39;rxjs&#39;;</div>
<div class="line">import { jobs } from &#39;@angular-devkit/core&#39;;</div>
<div class="line"> </div>
...
<p class="">Using the <code>createJobHandler()</code> helper, all those messages are automatically handled by the boilerplate code. If you need direct access to raw inputs, you should subscribe to the <code>context.inboundBus</code> Observable.</p>
<h3><a class="anchor" id="autotoc_md20314"></a>
Raw Output Stream</h3>
<p class="">The <code>Job&lt;&gt;</code> interface also contains a <code>outboundBus</code> member (of type <code>Observable&lt;JobOutboundMessage&lt;O&gt;&gt;</code> where <code>O</code> is the typed output of the job) which is the output complement of <code>inboundBus</code>. All messages sent <em>from</em> the job goes through this stream. The <code>kind</code> member of the <code>JobOutboundMessage&lt;O&gt;</code> interface dictates what kind of message it is sending:</p>
<ol type="1">
...
<p class="">Those messages can be accessed directly through the <code>job.outboundBus</code> member. The job itself should return an <code>Observable&lt;JobOutboundMessage&lt;O&gt;&gt;</code>. The <code>createJobHandler()</code> helper handles most of use cases of this and makes it easier for jobs to handle this.</p>
<h2><a class="anchor" id="autotoc_md20315"></a>
Job Dispatchers</h2>
<p class="">Dispatchers are a helper that redirect to different jobs given conditions. To create a job dispatcher, use the <code>createDispatcher()</code> function:</p>
<div class="fragment"><div class="line">import { jobs } from &#39;@angular-devkit/core&#39;;</div>
<div class="line"> </div>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md20316"></a>
Execution Strategy</h2>
<p class="">Jobs are always run in parallel and will always start, but many helper functions are provided when creating a job to help you control the execution strategy;</p>
<ol type="1">
...
<p class="">These strategies can be used when creating the job:</p>
...
<div class="line">  ),</div>
<div class="line">);</div>
</div><!-- fragment --><p class="">Strategies can be reused to synchronize between jobs. For example, given jobs <code>jobA</code> and <code>jobB</code>, you can reuse the strategy to serialize both jobs together;</p>
...
</div><!-- fragment --><p class="">Even further, we can have package A and package B run in serialization, and B and C also be serialized. Running A and C will run in parallel, while running B will wait for both A and C to finish.</p>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md20317"></a>
Scheduling Jobs</h1>
<p class="">Jobs can be scheduled using a <code>Scheduler</code> interface, which contains a <code>schedule()</code> method:</p>
...
<div class="line">}</div>
</div><!-- fragment --><p class="">The scheduler also has a <code>getDescription()</code> method to get a <code>JobDescription</code> object for a certain name; that description contains schemas for the argument, input, output, and other channels:</p>
<div class="fragment"><div class="line">interface Scheduler {</div>
<div class="line">  /**</div>
...
<div class="line">}</div>
</div><!-- fragment --><p class="">Finally, the scheduler interface has a <code>pause()</code> method to stop scheduling. This will queue all jobs and wait for the unpause function to be called before unblocking all the jobs scheduled. This does not affect already running jobs.</p>
<div class="fragment"><div class="line">interface Scheduler {</div>
<div class="line">  /**</div>
...
<div class="line">}</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md20318"></a>
&lt;a name="Dependencies"&gt;&lt;/a&gt;Synchronizing and Dependencies</h2>
<p class="">When scheduling jobs, it is often necessary to run jobs after certain other jobs are finished. This is done through the <code>dependencies</code> options in the <code>schedule()</code> method.</p>
<p class="">These jobs will also be passed to the job being scheduled, through its context. This can be useful if, for example, the output of those jobs are of a known type, or have known side channels.</p>
<p class="">An example of this would be a compiler that needs to know the output directory of other compilers before it, in a tool chain.</p>
<h3><a class="anchor" id="autotoc_md20319"></a>
Dependencies</h3>
<p class="">When scheduling jobs, the user can add a <code>dependencies</code> field to the scheduling options. The scheduler will wait for those dependencies to finish before running the job, and pass those jobs in the context of the job.</p>
<h3><a class="anchor" id="autotoc_md20320"></a>
Accessing Dependencies</h3>
<p class="">Jobs are called with a <code>JobHandlerContext</code> as a second argument, which contains a <code>dependencies: Job&lt;JsonValue&gt;[]</code> member which contains all dependencies that were used when scheduling the job. Those aren't fully typed as they are determined by the user, and not the job itself. They also can contain jobs that are not finished, and the job should use the <code>state</code> member of the job itself before trying to access its content.</p>
<h3><a class="anchor" id="autotoc_md20321"></a>
Scheduler Sub Jobs</h3>
<p class="">The <code>JobHandlerContext</code> also contains a <code>scheduler</code> member which can be used to schedule jobs using the same scheduler that was used for the job. This allows jobs to call other jobs and wait for them to end.</p>
<h2><a class="anchor" id="autotoc_md20322"></a>
Available Schedulers</h2>
<p class="">The Core Angular DevKit library provides 2 implementations for the <code>Scheduler</code> interface:</p>
<h2><a class="anchor" id="autotoc_md20323"></a>
SimpleJobRegistry</h2>
<p class="">Available in the jobs namespace. A registry that accept job registration, and can also schedule jobs.</p>
<div class="fragment"><div class="line">import { jobs } from &#39;@angular-devkit/core&#39;;</div>
<div class="line"> </div>
...
<div class="line"> </div>
<div class="line">scheduler.schedule(&#39;add&#39;, [1, 2, 3, 4]);</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md20324"></a>
NodeModuleJobRegistry</h2>
<p class="">Available through <code>@angular-devkit/core/node</code>.</p>
<p class="">A scheduler that loads jobs using their node package names. These jobs need to use the <code>createJobHandler()</code> helper and report their argument/input/output schemas that way.</p>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md20325"></a>
Gotchas</h1>
<ol type="1">
...
<h1><a class="anchor" id="autotoc_md20326"></a>
FAQ</h1>
<ol type="1">
<li>Laziness <br  />
