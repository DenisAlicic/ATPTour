...
<div class="contents">
<div class="textblock"><div align="center"></div><div align="center"><img src="logo/horizontal.png" alt="connect logo" width="450px" class="inline"/></div><div align="center"><a href="https://npmjs.org/package/connect"><img src="https://badgen.net/npm/v/connect" alt="NPM Version" class="inline"/></a> <a href="https://npmjs.org/package/connect"><img src="https://badgen.net/npm/dm/connect" alt="NPM Downloads" class="inline"/></a> <a href="https://travis-ci.org/senchalabs/connect"><img src="https://badgen.net/travis/senchalabs/connect/master" alt="Build Status" class="inline"/></a> <a href="https://coveralls.io/r/senchalabs/connect?branch=master"><img src="https://badgen.net/coveralls/c/github/senchalabs/connect/master" alt="Test Coverage" class="inline"/></a></div><div align="center"></div><p class="">Connect is an extensible HTTP server framework for <a href="http://nodejs.org">node</a> using "plugins" known as <em>middleware</em>.</p>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md8384"></a>
Getting Started</h2>
<p class="">Connect is a simple framework to glue together various "middleware" to handle requests.</p>
<h3><a class="anchor" id="autotoc_md8385"></a>
Install Connect</h3>
<div class="fragment"><div class="line">$ npm install connect</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md8386"></a>
Create an app</h3>
<p class="">The main component is a Connect "app". This will store all the middleware added and is, itself, a function.</p>
<div class="fragment"><div class="line">var app = connect();</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md8387"></a>
Use middleware</h3>
<p class="">The core of Connect is "using" middleware. Middleware are added as a "stack" where incoming requests will execute each middleware one-by-one until a middleware does not call <code>next()</code> within it.</p>
...
Mount middleware</h3>
<p class="">The <code>.use()</code> method also takes an optional path string that is matched against the beginning of the incoming request URL. This allows for basic routing.</p>
...
<div class="line">});</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md8390"></a>
Create a server from the app</h3>
<p class="">The last step is to actually use the Connect app in a server. The <code>.listen()</code> method is a convenience to start a HTTP server (and is identical to the <code>http.Server</code>'s <code>listen</code> method in the version of Node.js you are running).</p>
<div class="fragment"><div class="line">var server = app.listen(port);</div>
</div><!-- fragment --><p class="">The app itself is really just a function with three arguments, so it can also be handed to <code>.createServer()</code> in Node.js.</p>
<div class="fragment"><div class="line">var server = http.createServer(app);</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md8391"></a>
Middleware</h2>
<p class="">These middleware and libraries are officially supported by the Connect/Express team:</p>
<ul>
...
<p class="">Most of these are exact ports of their Connect 2.x equivalents. The primary exception is <code>cookie-session</code>.</p>
<p class="">Some middleware previously included with Connect are no longer supported by the Connect/Express team, are replaced by an alternative module, or should be superseded by a better module. Use one of these alternatives instead:</p>
<ul>
...
<p class="">Checkout <a href="https://github.com/Raynos/http-framework/wiki/Modules">http-framework</a> for many other compatible middleware!</p>
<h2><a class="anchor" id="autotoc_md8392"></a>
API</h2>
<p class="">The Connect API is very minimalist, enough to create an app and add a chain of middleware.</p>
<p class="">When the <code>connect</code> module is required, a function is returned that will construct a new app when called.</p>
...
<div class="line">var app = connect()</div>
</div><!-- fragment --><h3><a class="anchor" id="autotoc_md8393"></a>
app(req, res[, next])</h3>
<p class="">The <code>app</code> itself is a function. This is just an alias to <code>app.handle</code>.</p>
<h3><a class="anchor" id="autotoc_md8394"></a>
app.handle(req, res[, out])</h3>
<p class="">Calling the function will run the middleware stack against the given Node.js http request (<code>req</code>) and response (<code>res</code>) objects. An optional function <code>out</code> can be provided that will be called if the request (or error) was not handled by the middleware stack.</p>
<h3><a class="anchor" id="autotoc_md8395"></a>
app.listen([...])</h3>
<p class="">Start the app listening for requests. This method will internally create a Node.js HTTP server and call <code>.listen()</code> on it.</p>
<p class="">This is an alias to the <code>server.listen()</code> method in the version of Node.js running, so consult the Node.js documentation for all the different variations. The most common signature is <a href="https://nodejs.org/dist/latest-v6.x/docs/api/http.html#http_server_listen_port_hostname_backlog_callback"><code>app.listen(port)</code></a>.</p>
<h3><a class="anchor" id="autotoc_md8396"></a>
app.use(fn)</h3>
<p class="">Use a function on the app, where the function represents a middleware. The function will be invoked for every request in the order that <code>app.use</code> is called. The function is called with three arguments:</p>
...
<div class="line">})</div>
</div><!-- fragment --><p class="">In addition to a plan function, the <code>fn</code> argument can also be a Node.js HTTP server instance or another Connect app instance.</p>
<h3><a class="anchor" id="autotoc_md8397"></a>
app.use(route, fn)</h3>
<p class="">Use a function on the app, where the function represents a middleware. The function will be invoked for every request in which the URL (<code>req.url</code> property) starts with the given <code>route</code> string in the order that <code>app.use</code> is called. The function is called with three arguments:</p>
