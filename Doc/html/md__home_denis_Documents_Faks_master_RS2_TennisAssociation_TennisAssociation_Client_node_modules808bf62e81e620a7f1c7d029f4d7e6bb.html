...
<title>Tennis Association: @npmcli/git</title>
...
<div class="title">@npmcli/git </div>  </div>
</div><!--header-->
<div class="contents">
<div class="textblock"><p class="">A utility for spawning git from npm CLI contexts.</p>
<p class="">This is <em>not</em> an implementation of git itself, it's just a thing that spawns child processes to tell the system git CLI implementation to do stuff.</p>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md15497"></a>
API</h1>
<p class="">Most methods take an options object. Options are described below.</p>
<h2><a class="anchor" id="autotoc_md15498"></a>
&lt;tt&gt;git.spawn(args, opts = {})&lt;/tt&gt;</h2>
<p class="">Launch a <code>git</code> subprocess with the arguments specified.</p>
<p class="">All the other functions call this one at some point.</p>
<p class="">Processes are launched using <a href="http://npm.im/@npmcli/promise-spawn"><code>@npmcli/promise-spawn</code></a>, with the <code>stdioString: true</code> option enabled by default, since git output is generally in readable string format.</p>
<p class="">Return value is a <code>Promise</code> that resolves to a result object with <code>{cmd, args, code, signal, stdout, stderr}</code> members, or rejects with an error with the same fields, passed back from <a href="http://npm.im/@npmcli/promise-spawn"><code>@npmcli/promise-spawn</code></a>.</p>
<h2><a class="anchor" id="autotoc_md15499"></a>
‘git.clone(repo, ref = 'HEAD’, target = null, opts = {})&lt;tt&gt;-&gt;&lt;/tt&gt;Promise&lt;sha String&gt;`</h2>
<p class="">Clone the repository into <code>target</code> path (or the default path for the name of the repository), checking out <code>ref</code>.</p>
<p class="">Return value is the sha of the current HEAD in the locally cloned repository.</p>
<p class="">In lieu of a specific <code>ref</code>, you may also pass in a <code>spec</code> option, which is a <a href="http://npm.im/npm-package-arg"><code>npm-package-arg</code></a> object for a <code>git</code> package dependency reference. In this way, you can select SemVer tags within a range, or any git committish value. For example:</p>
...
<div class="line">})</div>
</div><!-- fragment --><p class="">This will automatically do a shallow <code>--depth=1</code> clone on any hosts that are known to support it. To force a shallow or deep clone, you can set the <code>gitShallow</code> option to <code>true</code> or <code>false</code> respectively.</p>
<h2><a class="anchor" id="autotoc_md15500"></a>
&lt;tt&gt;git.revs(repo, opts = {})&lt;/tt&gt; -&gt; &lt;tt&gt;Promise&lt;rev doc Object&gt;&lt;/tt&gt;</h2>
<p class="">Fetch a representation of all of the named references in a given repository. The resulting doc is intentionally somewhat <a href="https://www.npmjs.com/package/pacote#packuments">packument</a>-like, so that git semver ranges can be applied using the same <a href="http://npm.im/npm-pick-manifest"><code>npm-pick-manifest</code></a> logic.</p>
<p class="">The resulting object looks like:</p>
...
<div class="line">  },</div>
<div class="line">}</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md15501"></a>
&lt;tt&gt;git.is(opts)&lt;/tt&gt; -&gt; &lt;tt&gt;Promise&lt;Boolean&gt;&lt;/tt&gt;</h2>
<p class="">Resolve to <code>true</code> if the path argument refers to the root of a git repository.</p>
<p class="">It does this by looking for a file in <code>${path}/.git/index</code>, which is not an airtight indicator, but at least avoids being fooled by an empty directory or a file named <code>.git</code>.</p>
<h2><a class="anchor" id="autotoc_md15502"></a>
&lt;tt&gt;git.find(opts)&lt;/tt&gt; -&gt; &lt;tt&gt;Promise&lt;String | null&gt;&lt;/tt&gt;</h2>
<p class="">Given a path, walk up the file system tree until a git repo working directory is found. Since this calls <code>stat</code> a bunch of times, it's probably best to only call it if you're reasonably sure you're likely to be in a git project somewhere.</p>
<p class="">Resolves to <code>null</code> if not in a git project.</p>
<h2><a class="anchor" id="autotoc_md15503"></a>
&lt;tt&gt;git.isClean(opts = {})&lt;/tt&gt; -&gt; &lt;tt&gt;Promise&lt;Boolean&gt;&lt;/tt&gt;</h2>
<p class="">Return true if in a git dir, and that git dir is free of changes. This will resolve <code>true</code> if the git working dir is clean, or <code>false</code> if not, and reject if the path is not within a git directory or some other error occurs.</p>
