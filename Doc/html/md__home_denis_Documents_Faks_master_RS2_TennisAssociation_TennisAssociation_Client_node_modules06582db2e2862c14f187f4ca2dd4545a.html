...
<title>Tennis Association: minipass</title>
...
<div class="contents">
<div class="textblock"><p class="">A <em>very</em> minimal implementation of a <a href="https://nodejs.org/api/stream.html#stream_class_stream_passthrough">PassThrough stream</a></p>
<p class=""><a href="https://docs.google.com/spreadsheets/d/1oObKSrVwLX_7Ut4Z6g3fZW-AX1j1-k6w-cDsrkaSbHM/edit#gid=0">It's very fast</a> for objects, strings, and buffers.</p>
<p class="">Supports pipe()ing (including multi-pipe() and backpressure transmission), buffering data until either a <code>data</code> event handler or <code>pipe()</code> is added (so you don't lose the first chunk), and most other cases where PassThrough is a good idea.</p>
<p class="">There is a <code>read()</code> method, but it's much more efficient to consume data from this stream via &lsquo;'data&rsquo;<code>events or by calling</code>pipe()<code>into some other stream. Calling</code>read()` requires the buffer to be flattened in some cases, which requires copying memory.</p>
<p class="">There is also no <code>unpipe()</code> method. Once you start piping, there is no stopping it!</p>
<p class="">If you set <code>objectMode: true</code> in the options, then whatever is written will be emitted. Otherwise, it'll do a minimal amount of Buffer copying to ensure proper Streams semantics when <code>read(n)</code> is called.</p>
<p class=""><code>objectMode</code> can also be set by doing <code>stream.objectMode = true</code>, or by writing any non-string/non-buffer data. <code>objectMode</code> cannot be set to false once it is set.</p>
<p class="">This is not a <code>through</code> or <code>through2</code> stream. It doesn't transform the data, it just passes it right through. If you want to transform the data, extend the class, and override the <code>write()</code> method. Once you're done transforming the data however you want, call <code>super.write()</code> with the transform output.</p>
<p class="">For some examples of streams that extend Minipass in various ways, check out:</p>
<ul>
...
<h1><a class="anchor" id="autotoc_md22602"></a>
Differences from Node.js Streams</h1>
<p class="">There are several things that make Minipass streams different from (and in some ways superior to) Node.js core streams.</p>
<p class="">Please read these caveats if you are familiar with noode-core streams and intend to use Minipass streams in your programs.</p>
<h2><a class="anchor" id="autotoc_md22603"></a>
Timing</h2>
<p class="">Minipass streams are designed to support synchronous use-cases. Thus, data is emitted as soon as it is available, always. It is buffered until read, but no longer. Another way to look at it is that Minipass streams are exactly as synchronous as the logic that writes into them.</p>
<p class="">This can be surprising if your code relies on <code>PassThrough.write()</code> always providing data on the next tick rather than the current one, or being able to call <code>resume()</code> and not have the entire buffer disappear immediately.</p>
<p class="">However, without this synchronicity guarantee, there would be no way for Minipass to achieve the speeds it does, or support the synchronous use cases that it does. Simply put, waiting takes time.</p>
<p class="">This non-deferring approach makes Minipass streams much easier to reason about, especially in the context of Promises and other flow-control mechanisms.</p>
<h2><a class="anchor" id="autotoc_md22604"></a>
No High/Low Water Marks</h2>
<p class="">Node.js core streams will optimistically fill up a buffer, returning <code>true</code> on all writes until the limit is hit, even if the data has nowhere to go. Then, they will not attempt to draw more data in until the buffer size dips below a minimum value.</p>
<p class="">Minipass streams are much simpler. The <code>write()</code> method will return <code>true</code> if the data has somewhere to go (which is to say, given the timing guarantees, that the data is already there by the time <code>write()</code> returns).</p>
<p class="">If the data has nowhere to go, then <code>write()</code> returns false, and the data sits in a buffer, to be drained out immediately as soon as anyone consumes it.</p>
<h2><a class="anchor" id="autotoc_md22605"></a>
Hazards of Buffering (or: Why Minipass Is So Fast)</h2>
<p class="">Since data written to a Minipass stream is immediately written all the way through the pipeline, and <code>write()</code> always returns true/false based on whether the data was fully flushed, backpressure is communicated immediately to the upstream caller. This minimizes buffering.</p>
<p class="">Consider this case:</p>
...
</div><!-- fragment --><p class="">Along the way, the data was buffered and deferred at each stage, and multiple event deferrals happened, for an unblocked pipeline where it was perfectly safe to write all the way through!</p>
<p class="">Furthermore, setting a <code>highWaterMark</code> of <code>1024</code> might lead someone reading the code to think an advisory maximum of 1KiB is being set for the pipeline. However, the actual advisory buffering level is the <em>sum</em> of <code>highWaterMark</code> values, since each one has its own bucket.</p>
<p class="">Consider the Minipass case:</p>
...
</div><!-- fragment --><p class="">It is extremely unlikely that you <em>don't</em> want to buffer any data written, or <em>ever</em> buffer data that can be flushed all the way through. Neither node-core streams nor Minipass ever fail to buffer written data, but node-core streams do a lot of unnecessary buffering and pausing.</p>
<p class="">As always, the faster implementation is the one that does less stuff and waits less time to do it.</p>
<h2><a class="anchor" id="autotoc_md22606"></a>
Immediately emit &lt;tt&gt;end&lt;/tt&gt; for empty streams (when not paused)</h2>
<p class="">If a stream is not paused, and <code>end()</code> is called before writing any data into it, then it will emit <code>end</code> immediately.</p>
<p class="">If you have logic that occurs on the <code>end</code> event which you don't want to potentially happen immediately (for example, closing file descriptors, moving on to the next entry in an archive parse stream, etc.) then be sure to call <code>stream.pause()</code> on creation, and then <code>stream.resume()</code> once you are ready to respond to the <code>end</code> event.</p>
<h2><a class="anchor" id="autotoc_md22607"></a>
Emit &lt;tt&gt;end&lt;/tt&gt; When Asked</h2>
<p class="">One hazard of immediately emitting &lsquo;'end&rsquo;` is that you may not yet have had a chance to add a listener. In order to avoid this hazard, Minipass streams safely re-emit the &lsquo;'end&rsquo;<code>event if a new listener is added after </code>'end'` has been emitted.</p>
<p class="">Ie, if you do &lsquo;stream.on('end&rsquo;, someFunction)<code>, and the stream has already emitted</code>end<code>, then it will call the handler right away. (You can think of this somewhat like attaching a new</code>.then(fn)` to a previously-resolved Promise.)</p>
<p class="">To prevent calling handlers multiple times who would not expect multiple ends to occur, all listeners are removed from the &lsquo;'end&rsquo;` event whenever it is emitted.</p>
<h2><a class="anchor" id="autotoc_md22608"></a>
Impact of "immediate flow" on Tee-streams</h2>
<p class="">A "tee stream" is a stream piping to multiple destinations:</p>
...
</div><!-- fragment --><p class="">Since Minipass streams <em>immediately</em> process any pending data through the pipeline when a new pipe destination is added, this can have surprising effects, especially when a stream comes in from some other function and may or may not have data in its buffer.</p>
...
</div><!-- fragment --><p class="">The solution is to create a dedicated tee-stream junction that pipes to both locations, and then pipe to <em>that</em> instead.</p>
...
</div><!-- fragment --><p class="">The same caveat applies to &lsquo;on('data&rsquo;)` event listeners. The first one added will <em>immediately</em> receive all of the data, leaving nothing for the second:</p>
<div class="fragment"><div class="line">// WARNING! WILL LOSE DATA!</div>
<div class="line">const src = new Minipass()</div>
<div class="line">src.write(&#39;foo&#39;)</div>
...
</div><!-- fragment --><p class="">Using a dedicated tee-stream can be used in this case as well:</p>
...
</div><!-- fragment --><h1><a class="anchor" id="autotoc_md22609"></a>
USAGE</h1>
<p class="">It's a stream! Use it like a stream and it'll most likely do what you want.</p>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22610"></a>
OPTIONS</h2>
<ul>
...
<h2><a class="anchor" id="autotoc_md22611"></a>
API</h2>
<p class="">Implements the user-facing portions of Node.js's <code>Readable</code> and <code>Writable</code> streams.</p>
...
EXAMPLES</h1>
<p class="">Here are some examples of things you can do with Minipass streams.</p>
...
<div class="line">})</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22618"></a>
collecting</h2>
...
<div class="line">})</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22619"></a>
collecting into a single blob</h2>
<p class="">This is a bit slower because it concatenates the data into one chunk for you, but if you're going to do it yourself anyway, it's convenient this way:</p>
...
<div class="line">})</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22620"></a>
iteration</h2>
<p class="">You can iterate over streams synchronously or asynchronously in platforms that support it.</p>
<p class="">Synchronous iteration will end when the currently available data is consumed, even if the <code>end</code> event has not been reached. In string and buffer mode, the data is concatenated, so unless multiple writes are occurring in the same tick as the <code>read()</code>, sync iteration loops will generally only have a single iteration.</p>
<p class="">To consume chunks in this way exactly as they have been written, with no flattening, create the stream with the <code>{ objectMode: true }</code> option.</p>
...
<div class="line">}</div>
</div><!-- fragment --><p class="">Asynchronous iteration will continue until the end event is reached, consuming all of the data.</p>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22621"></a>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22622"></a>
...
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22623"></a>
...
<div class="line">  }</div>
<div class="line">}</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22624"></a>
...
<div class="line">  }</div>
<div class="line">}</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md22625"></a>
