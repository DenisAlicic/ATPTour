...
API</h1>
<p class="">The only exposed method is a constructor (<code>JestWorker</code>) that is initialized by passing the worker path, plus an options object.</p>
...
Methods</h2>
<p class="">The returned <code>JestWorker</code> instance has all the exposed methods, plus some additional ones to interact with the workers itself:</p>
