This document describes operations performed to produce, or parse, the flatted output.\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_flatted_SPECS_autotoc_md24008}{}\doxysection{stringify(any) =$>$ flatted\+String}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_flatted_SPECS_autotoc_md24008}
...
The stringified indexes ensure there won\textquotesingle{}t be conflicts with regularly stored numbers.\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_flatted_SPECS_autotoc_md24009}{}\doxysection{parse(flatted\+String) =$>$ any}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_flatted_SPECS_autotoc_md24009}
...
That is the key to place back values from the rest of the array, so that per each property of the object at index {\ttfamily 0}, if the value is an {\ttfamily instanceof} String, something not serializable via JSON, it means it can be used to retrieve the position of its value from the {\ttfamily input} array.
