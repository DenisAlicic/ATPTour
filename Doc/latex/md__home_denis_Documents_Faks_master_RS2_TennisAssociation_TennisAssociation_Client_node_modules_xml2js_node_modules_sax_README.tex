...
Designed with \href{http://nodejs.org/}{\texttt{ node}} in mind, but should work fine in the browser or other Common\+JS implementations.\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31685}{}\doxysection{What This Is}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31685}
...
\end{DoxyItemize}\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31686}{}\doxysection{What This Is (probably) Not}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31686}
...
\end{DoxyItemize}\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31687}{}\doxysection{Regarding $<$tt$>$$<$!\+DOCTYPE$<$/tt$>$s and $<$tt$>$$<$!\+ENTITY$<$/tt$>$s}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31687}
The parser will handle the basic XML entities in text nodes and attribute values\+: {\ttfamily \& $<$ $>$ \textquotesingle{} "{}}. It\textquotesingle{}s possible to define additional entities in XML by putting them in the DTD. This parser doesn\textquotesingle{}t do anything with that. If you want to listen to the {\ttfamily ondoctype} event, and then fetch the doctypes, and read the entities and add them to {\ttfamily parser.\+ENTITIES}, then be my guest.

Unknown entities will fail in strict mode, and in loose mode, will pass through unmolested.\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31688}{}\doxysection{Usage}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31688}
...
\end{DoxyCode}
\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31689}{}\doxysection{Arguments}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31689}
...
\item {\ttfamily strict\+Entities} -\/ Boolean. If true, only parse \href{http://www.w3.org/TR/REC-xml/\#sec-predefined-ent}{\texttt{ predefined XML entities}} ({\ttfamily \&}, {\ttfamily \textquotesingle{}}, {\ttfamily $>$}, {\ttfamily $<$}, and {\ttfamily "{}})
\end{DoxyItemize}\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31690}{}\doxysection{Methods}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31690}
...
{\ttfamily resume} -\/ To gracefully handle errors, assign a listener to the {\ttfamily error} event. Then, when the error is taken care of, you can call {\ttfamily resume} to continue parsing. Otherwise, the parser will not continue while in an error state.\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31691}{}\doxysection{Members}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31691}
...
And a bunch of other stuff that you probably shouldn\textquotesingle{}t touch.\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31692}{}\doxysection{Events}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31692}
All events emit with a single argument. To listen to an event, assign a function to {\ttfamily on$<$eventname$>$}. Functions get executed in the this-\/context of the parser object. The list of supported events are also in the exported {\ttfamily EVENTS} array.
...
{\ttfamily doctype} -\/ The {\ttfamily $<$!\+DOCTYPE} declaration. Argument\+: doctype string.
...
{\ttfamily sgmldeclaration} -\/ Random SGML declarations. Stuff like {\ttfamily $<$!\+ENTITY p$>$} would trigger this kind of event. This is a weird thing to support, so it might go away at some point. SAX isn\textquotesingle{}t intended to be used to parse SGML, after all.
...
{\ttfamily opencdata} -\/ The opening tag of a {\ttfamily $<$!\mbox{[}CDATA\mbox{[}} block.

{\ttfamily cdata} -\/ The text of a {\ttfamily $<$!\mbox{[}CDATA\mbox{[}} block. Since {\ttfamily $<$!\mbox{[}CDATA\mbox{[}} blocks can get quite large, this event may fire multiple times for a single block, if it is broken up into multiple {\ttfamily write()}s. Argument\+: the string of random character data.

{\ttfamily closecdata} -\/ The closing tag ({\ttfamily \mbox{]}\mbox{]}$>$}) of a {\ttfamily $<$!\mbox{[}CDATA\mbox{[}} block.
...
{\ttfamily noscript} -\/ In non-\/strict mode, {\ttfamily $<$script$>$} tags trigger a {\ttfamily \char`\"{}script\char`\"{}} event, and their contents are not checked for special xml characters. If you pass {\ttfamily noscript\+: true}, then this behavior is suppressed.\hypertarget{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31693}{}\doxysection{Reporting Problems}\label{md__home_denis_Documents_Faks_master_RS2_TennisAssociation_TennisAssociation_Client_node_modules_xml2js_node_modules_sax_README_autotoc_md31693}
